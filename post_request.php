...
if (isset($_POST['submit'])) {
	$judul_request = $_POST['judul_request'];
	$detail_request = $_POST['detail_request'];
    $biaya_request = $_POST['biaya_request'];
    $result = mysqli_query($conn, $sql);
		
    if (!$result->num_rows > 0) {
         $sql = "INSERT INTO tb_request (judul_request, detail_request, biaya_request)
            VALUES ('$judul_request', '$detail_request', '$biaya_request')";
        $result = mysqli_query($conn, $sql);
        if ($result) {
            echo "<script>alert('Request Berhasil di-Submit!.')</script>";
            echo '<script>window.location="manage_request.php"</script>';
            $judul_request = "";
            $detail_request = "";
            $biaya_request = "";
        } 
